...
interface Props {
  ...
  onReorderBlocks: (items: ServiceDetailBlock[]) => Promise<void> | void
}

/**
 * Tab "Blocos de detalhe" — CRUD da lista de `ServiceDetailBlock`, com
 * drag & drop, toggle de visibilidade e edição inline com save no blur.
 *
...
export default function TabBlocks({
  ...
  return (
    <div className="space-y-3">
      ...
      <DragList
        items={[...blocks].sort((a, b) => a.order - b.order)}
        getKey={(b) => b.id}
        onReorder={(next) => onReorderBlocks(next)}
        className="space-y-3"
      >
        {(block, handle) => (
